    ...
    <main className={styles.container}>
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          const pairs: string[] =
            (e.target as any).select.length == undefined
              ? [(e.target as any).select.value]
              : Array.from((e.target as any).select).map(
                  (pair: any) => pair.value
                );
...
export async function getStaticProps() {
  ...
  const assetPairs = Object.values((await res.json()).result).map(
    (value: any) => ({ value: value.wsname, label: value.wsname })
  );
  return {
    props: {
      assetPairs,
