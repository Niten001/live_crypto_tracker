...
export default function CryptoChart() {
  ...
  useEffect(() => {
    if (socket == undefined) {
      ...
      socket.onopen = () => {
        let tempPairs: string[] = JSON.parse(localStorage.getItem("pairs") ?? "[]");
        socket.send(`{
          "event": "subscribe",
  ...
  return (<>
    {pairs.map((pair, index) => {
      let color = (((index + 103) * 3914269) % 0xFFFFFF).toString(16).padStart(6, '0');
      return (<div key={index} style={{height: `${100 / (pairs.length)}%`}}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip />
            <Legend />
            <Line connectNulls dataKey={pair} stroke={`#${color}`} type="linear" />
          </LineChart>
        </ResponsiveContainer>
      </div>);
    })}
