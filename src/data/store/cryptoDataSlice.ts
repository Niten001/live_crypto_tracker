...
export const cryptoDataSlice = createSlice({
  ...
  reducers: {
    addCryptoData: (state, action) => {
      if (
        state.value.filter(
          (point) => Object.keys(point)[1] == Object.keys(action.payload)[1]
        ).length >= 50
      ) {
        state.value.splice(
    ...
    initCryptoData: (state) => {
      ...
      state.value = cryptoData.filter((point: any) =>
        pairs.includes(Object.keys(point)[1])
      );
      localStorage.setItem("cryptoData", JSON.stringify(state.value));
    },
  },
...
export const getCryptoData = (state: { cryptoData: CryptoDataState }) =>
